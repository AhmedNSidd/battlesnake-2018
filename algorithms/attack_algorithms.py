from .utils import get_manhattan_distance, translate, generate_data_dictionary
from .graph_algorithms import a_star, bfs
from copy import deepcopy
from .board import Board
from .constants import EMPTY_SPACE_MAKERS
from heapq import heappush, heappop
from .routes import path_to_tail_exists
...
def attack_enemy(board):
    ...
    for snake in board.other_snakes:
        ...
        heappush(attack_points, (get_manhattan_distance(
                                    board.my_snake.get_head(), neighbours[0]),
                                    neighbours[0]))
    ...
    while attack_points:
        ...
        if cost_of_enemy != None:
            return ("Attacking", translate(board.my_snake.get_head(),
                                        path_to_enemy[1]))
    return (None, None)
...
def cornering_enemies(board):
    ...
    for snake in board.other_snakes:
        ...
        if (enemy_distance < get_manhattan_distance(
                                    board.my_snake.get_head(), exit_node)):
            exit_node = curr_node
        if (enemy_distance < get_manhattan_distance(
                                    board.my_snake.get_head(), exit_node)):
        ...
        if (distance_of_samaritan_to_exit < enemy_distance
            or (distance_of_samaritan_to_exit == enemy_distance
                and snake.length < board.my_snake.length)):
            return ("Cornering", translate(board.my_snake.get_head(),
            ...
            enemy_distance = get_manhattan_distance(snake.get_head(),
                                                    exit_node)
            samaritan_cost, samaritan_path = a_star(board,
                    board.my_snake.get_head(), exit_node, board.my_snake)
            # removed "or enemy_distance == None" from below
            if (samaritan_cost == None or len(samaritan_path) == 1):
                continue
            distance_of_samaritan_to_exit = len(samaritan_path) - 1
...
def trapping_enemies(board):
    ...
    for snake in board.other_snakes:
        ...
        my_direction = translate(board.my_snake.coordinates[1],
                                    board.my_snake.get_head())
        move = None
        if xcoord == board.width-1:
            ...
            if my_direction in permissible_directions_for_samaritan:
                if (board.my_snake.get_head() == (board.width-2, ycoord)
                    and board.is_valid_move(direction_of_enemy)):
                    move = direction_of_enemy
                elif (board.my_snake.get_head() == (board.width-3, ycoord)
                        and board.is_valid_move("right")
                        and snake.length < board.my_snake.length):
                        move = "right"
                elif (board.my_snake.get_head() == (board.width-2, ycoord-1)
            ...
            if my_direction in permissible_directions_for_samaritan:
                if (board.my_snake.get_head() == (1, ycoord)
                    and board.is_valid_move(direction_of_enemy)):
                    move = direction_of_enemy
                elif (board.my_snake.get_head() == (2, ycoord)
                        and board.is_valid_move("left")
                        and snake.length < board.my_snake.length):
                        move = "left"
                elif (board.my_snake.get_head() == (1, ycoord-1)
                        and direction_of_enemy == "down"
                        and snake.length < board.my_snake.length
                        and board.is_valid_move("down")):
                        move = "down"
            ...
            if my_direction in permissible_directions_for_samaritan:
                if (board.my_snake.get_head() == (xcoord, board.height-2)
                    and board.is_valid_move(direction_of_enemy)):
                    move = direction_of_enemy
                elif (board.my_snake.get_head() == (xcoord, board.height-3)
                        and board.is_valid_move("down")
                        and snake.length < board.my_snake.length):
                        move = "down"
                elif (board.my_snake.get_head() == (xcoord-1, board.height-2)
            ...
            if my_direction in permissible_directions_for_samaritan:
                if (board.my_snake.get_head() == (xcoord, 1)
                    and board.is_valid_move(direction_of_enemy)):
                    move = direction_of_enemy
                elif (board.my_snake.get_head() == (xcoord, 2)
                        and board.is_valid_move("up")
                        and snake.length < board.my_snake.length):
                        move = "up"
                elif (board.my_snake.get_head() == (xcoord-1, 1)
                        and direction_of_enemy == "right"
                        and snake.length < board.my_snake.length
                        and board.is_valid_move("right")):
                        move = "right"
...
def walling_enemies(board):
    ...
    for neighbour in neighbours:
        ...
        start = board.my_snake.get_head()
        destination = neighbour
        distance_to_edge = 0
        direction = translate(start, destination)
        valid_move = True
        path_to_edge = [board.my_snake.get_head()]
        foods = 0
        health_loss = 0
        while valid_move:
            ...
            if board.grid[target_y][target_x] != EMPTY_SPACE_MAKERS:
                if destination in food_coordinates:
                    ...
                    if destination not in my_snake.coordinates:
                        its_a_tail = False
            ...
            valid_move = board.is_valid_coordinate(destination[0],
                                                    ...
                                                    distance_to_edge+1,
                                                    foods_in_path=foods)
        my_snake.health -= health_loss
        new_snake_coords = []
        if my_snake.health == 100:
            my_snake.length += foods
            if my_snake.length-1 <= distance_to_edge:
    ...
    # did divided by 2 here to see if it works better, maybe change it back.
            ...
            my_snake.length += foods
            if my_snake.length <= distance_to_edge:
                for x in range(my_snake.length):
                    xcoord, ycoord = path_to_edge[-1-x]
                    new_snake_coords.append((xcoord, ycoord))
                my_snake.coordinates = new_snake_coords
            else:
    # did divided by 2 here to see if it works better, maybe change it back.
        ...
        new_board = Board(generate_data_dictionary(board, food_coordinates,
        ...
        for x in range(len(new_board.other_snakes)):
            ...
            if not (bfs(new_board, future_enemy_snake.get_head(),
                    future_enemy_snake.get_tail(), future_enemy_snake)):
                if not (bfs(board, enemy_snake.get_head(),
                ...
                for node in path_to_edge[1:]:
                    ...
                    if my_distance_to_node > enemy_distance_to_node:
                        ...
                        break
                            ...
                            and enemy_snake.length >= board.my_snake.length):
                            can_wall_off_faster = False
                            break
...
def get_best_enemy_attack(board, move):
        ...
        for a_snake in new_other_snakes:
            if a_snake.id == snake.id:
                ...
                for neighbour in neighbours:
                    ...
                    new_board = Board(generate_data_dictionary(board,
                            new_foods, new_other_snakes, new_main_snake))
                    objective, move, enemy_id = ParanoiaStrategy.get_action(
                        new_board)
                    if board.my_snake.id == enemy_id:
                        return (objective, move, snake.id)
    return (None, None, None)

class ParanoiaStrategy():
