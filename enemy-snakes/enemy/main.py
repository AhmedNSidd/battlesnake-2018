...
def generate_grid(snake_id, my_snake_length, data):
    ...
    for snake in data['snakes']['data']:
        ...
        if snake_id != snake['id']:
            if my_snake_length <= len(snake['body']['data']):
                danger_spots = neighbours(point_to_list((snake['body']['data'][0]['x'],snake['body']['data'][0]['y'])), grid, PATH_FINDING_OBSTACLES)
...
def get_snake_tails(data):
    tails = []
    for snake in data['snakes']['data']:
        snake_tail = point_to_list((snake['body']['data'][-1]['x'],snake['body']['data'][-1]['y']))
...
def add_bad_moves_to_grid(my_snake_head, my_snake_id, snakes, grid):
    ...
    for enemy in snakes:
        if enemy['id'] == my_snake_id:
            continue
        neigh = neighbours(point_to_list((enemy['body']['data'][0]['x'],enemy['body']['data'][0]['y'])), grid, PATH_FINDING_OBSTACLES)
        for n in neigh:
...
def run_ai(data):
    ...
    my_snake_length = len(data['you']['body']['data'])
    ...
    my_snake_head = point_to_list((data['you']['body']['data'][0]['x'], data['you']['body']['data'][0]['y']))
    my_snake_tail = point_to_list((data['you']['body']['data'][-1]['x'],data['you']['body']['data'][-1]['y']))
    ...
    for goal in goals:
        ...
        for snake in snakes:
            if(snake['id'] != snake_id):
                enemy_dist = distance(point_to_list((snake['body']['data'][0]['x'],snake['body']['data'][0]['y'])), goal)
...
def path_to_enemy_tail(my_snake_head, snake_id, snakes, waypoints, links, grid):
    ...
    for snake in snakes:
        if snake_id == snake['id']:
            continue
        enemy_tail = point_to_list((snake['body']['data'][-1]['x'],snake['body']['data'][-1]['y']))
...
def corner_enemy(my_snake_head, my_snake_length, my_snake_id, snakes, waypoints, links, grid):
    ...
    for snake in snakes:
        ...
        enemy_head = point_to_list((snake['body']['data'][0]['x'],snake['body']['data'][0]['y']))
        ...
        if(path_distance(my_path) <= enemy_dist and my_snake_length > len(snake['body']['data'])):
...
def path_to_bully_enemy(my_snake_head, my_snake_length, snake_id, goals, snakes, waypoints, links, grid, my_snake_overlapping):
    ...
    for snake in snakes:
        ...
        if my_snake_length > len(snake['body']['data']):
            ...
            head_neighbours = []
        forward = get_forward_node(enemy_head, point_to_list((snake['body']['data'][1]['x'], snake['body']['data'][1]['y'])), grid)
